...
void key_attach(Key *key, KEY_EVENT e, key_cb_t cb) { key->key_cb[e] = cb; }
...
void advanced_key_update(AdvancedKey *advanced_key, float value) {
  ...
  case KEY_ANALOG_NORMAL_MODE:
    ...
    if (advanced_key->value - advanced_key->schmitt_parameter >
        advanced_key->activation_value) {
      state = true;
    }
    if (advanced_key->value + advanced_key->schmitt_parameter <
        advanced_key->activation_value) {
  ...
  case KEY_ANALOG_RAPID_MODE:
    ...
    if (advanced_key->key.state) {
      if (advanced_key->value - advanced_key->minimum >=
          advanced_key->trigger_distance) {
        if (advanced_key->value > advanced_key->maximum)
          advanced_key->maximum = advanced_key->value;
      }
      if (advanced_key->maximum - advanced_key->value >=
          advanced_key->release_distance) {
        advanced_key->minimum = advanced_key->value;
        state = false;
      }
    } else {
...
void advanced_key_set_deadzone(AdvancedKey *key, float upper, float lower) {
  ...
  key->range = (key->upper_bound - key->upper_deadzone) -
  (key->lower_bound+key->lower_deadzone);
  */
  key->upper_deadzone = upper;
  key->lower_deadzone = lower;
  key->range = (key->upper_bound -
                (key->upper_bound - key->lower_bound) * key->upper_deadzone) -
               (key->lower_bound +
                (key->upper_bound - key->lower_bound) * key->lower_deadzone);
}
